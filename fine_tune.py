...
"""
Fine-tuning the library gpt2_orig for language modeling on a text file (GPT, GPT-2, CTRL, BERT, RoBERTa, XLNet).
...
class ModelArguments:
    ...
    cache_dir: Optional[str] = field(
        default=None, metadata={"help": "Where do you want to store the pretrained gpt2_orig downloaded from s3"}
    )
...
class DataTrainingArguments:
    ...
    train_data_file: Optional[str] = field(
        default='train_test_splits/train.txt',
        metadata={"help": "The input training data file (a text file)."}
    )
    eval_data_file: Optional[str] = field(
        default='train_test_splits/val.txt',
...
def main():
    ...
    if config.model_type in ["bert", "roberta", "distilbert", "camembert"] and not data_args.mlm:
        raise ValueError(
            "BERT and RoBERTa-like gpt2_orig do not have LM heads but masked LM heads. They must be run using the"
    ...
    if training_args.do_train:
        ...
        # so that you can share your model easily on huggingface.co/gpt2_orig =)
